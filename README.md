...
| Job info access | SPANK API (```spank_get_item```, etc.) to read C structs | ```pbs.event().job``` object — attributes readable and writable directly | Many ways to request job info. ```JournalConsumer``` is the event-driven way. |
| Job rejection / modification | [Limited] Return an error code to abort; modifying attributes is difficult | [Flexible] ```event.reject()``` and attribute rewriting are straightforward | Rejection: jobtap plugin specifically the validation callback [```flux_jobtap_reject_job```[(https://flux-framework.readthedocs.io/projects/flux-core/en/latest/man3/flux_jobtap_get_flux.html). Modification: [frobnicator plugins](https://flux-framework.readthedocs.io/projects/flux-core/en/latest/python/autogenerated/flux.job.frobnicator.frobnicator.html). |
